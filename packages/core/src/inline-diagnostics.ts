...
interface RuleCheck {
  /** Rule identifier */
  ruleId?: string;
  /** Human-readable label for the violation */
  message: string;
  ruleFile: string;
  severity: Diagnostic['severity'];
  /** Test a single line. Return true if violation found on this line. */
  test: (line: string, lineIndex: number, allLines: string[]) => boolean;
  /** Optional: narrow column range (defaults to 0..-1) */
  getRange?: (line: string) => [number, number];
}

// ─── SOQL-in-loop / DML-in-loop detection ─────────────────────────────────
// We track loop depth and flag any SOQL SELECT or DML found inside a loop block.

function apexDiagnostics(lines: string[]): Diagnostic[] {
  const diagnostics: Diagnostic[] = [];
  let loopDepth = 0;
  let braceDepth = 0;
  // Track brace depths when a loop opened so we can close properly
  const loopOpenDepths: number[] = [];
...
function testSetupDiagnostics(lines: string[]): Diagnostic[] {
  const diagnostics: Diagnostic[] = [];
  ...
  if (classLine === -1) return diagnostics;

  const hasTestSetup = lines.some((l) => /@TestSetup\b/i.test(l));
  if (hasTestSetup) return diagnostics;
...
function nakedCatchDiagnostics(lines: string[]): Diagnostic[] {
  const diagnostics: Diagnostic[] = [];
...
const SIMPLE_CHECKS: RuleCheck[] = [
  // ── without sharing class declaration (no sharing keyword at all) ──────────
  {
    ruleId: 'missing-sharing-declaration',
    message:
      'Class declared without sharing — use `with sharing` by default. See .cursor/rules/apex.mdc.',
    ruleFile: '.cursor/rules/apex.mdc',
    severity: 'warning',
    test: (line) => {
      if (!/\bclass\b/.test(line)) return false;
      if (/\bwith sharing\b|\bwithout sharing\b|\binherited sharing\b/.test(line)) return false;
  ...
  // Flags `without sharing` unless the previous line has an explanatory comment.
  {
    ...
    message:
      '`without sharing` detected without an explanatory comment on the previous line. Add a comment explaining why this bypass is intentional.',
    ruleFile: '.cursor/rules/apex.mdc',
    severity: 'warning',
    test: (line, i, all) => {
      if (!/\bwithout sharing\b/.test(line)) return false;
      if (!/\bclass\b/.test(line)) return false;
      // Check previous line for a comment
      if (i === 0) return true; // no previous line
  ...
  // ── Hardcoded Salesforce-style IDs ────────────────────────────────────────
  {
    ...
    message:
      'Possible hardcoded Salesforce ID — use Custom Metadata or pass IDs as parameters. See .cursor/rules/apex.mdc.',
    ruleFile: '.cursor/rules/apex.mdc',
    severity: 'warning',
    test: (line) => {
  ...
  // Expanded: also flags password, token, key, secret, jwt, ssn, credit
  {
    ruleId: 'no-debug-pii',
    message:
      'Debug statement may log sensitive data — ensure no PII or credentials are included. See .cursor/rules/safety.mdc.',
    ruleFile: '.cursor/rules/safety.mdc',
    severity: 'warning',
    test: (line) =>
      /System\.debug/i.test(line) &&
  ...
  // ── no-seealldata ─────────────────────────────────────────────────────────
  {
    ...
    message:
      'SeeAllData=true is dangerous — tests should create their own data. Remove SeeAllData=true. See .cursor/rules/apex.mdc.',
    ruleFile: '.cursor/rules/apex.mdc',
    severity: 'error',
...
function lwcJsDiagnostics(lines: string[]): Diagnostic[] {
  const diagnostics: Diagnostic[] = [];
  ...
  if (hasWire) {
    ...
    const hasErrorHandling =
      /\{\s*data\s*,\s*error\s*\}/.test(fullContent) ||
...
function lwcHtmlDiagnostics(lines: string[]): Diagnostic[] {
  const diagnostics: Diagnostic[] = [];
...
export function detectFileType(filePath: string): FileType {
  if (filePath.endsWith('.cls') || filePath.endsWith('.trigger')) return 'apex';
  if (filePath.endsWith('.js') && filePath.includes('/lwc/')) return 'lwc-js';
  if (filePath.endsWith('.html') && filePath.includes('/lwc/')) return 'lwc-html';
  return 'unknown';
}

export function analyseFile(content: string, fileType: FileType): Diagnostic[] {
    ...
    for (let i = 0; i < lines.length; i++) {
      for (const check of SIMPLE_CHECKS) {
        if (check.test(lines[i], i, lines)) {
          const range = check.getRange ? check.getRange(lines[i]) : ([0, -1] as [number, number]);
          diagnostics.push({
            line: i,
            startCol: range[0],
            endCol: range[1],
